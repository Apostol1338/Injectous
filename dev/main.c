...
read_JPEG_file (char* filename) {
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    //fwrite(buffer[i++], row_stride, 7, output);
  }
...
GLOBAL(int)
read_DCT_coef(const char* filename, const char* output_filename) {
  struct jpeg_decompress_struct cinfo;
  struct my_error_mgr jerr;
  FILE* infile;
  FILE* outfile;
  ...
  for (JDIMENSION compNum=0; compNum < cinfo.num_components; compNum++) {
    ...
    for (JDIMENSION rowNum=0; rowNum < cinfo.comp_info[compNum].height_in_blocks; rowNum++) {
      ...
      rowPtrs[compNum] = ((&cinfo)->mem->access_virt_barray)((j_common_ptr) &cinfo, src_coef_arrays[compNum],rowNum, (JDIMENSION) 1, FALSE);
      ...
      for (JDIMENSION blockNum=0; blockNum < cinfo.comp_info[compNum].width_in_blocks; blockNum++){
        ...
        for (JDIMENSION i=0; i<DCTSIZE2; i++){
          ...
          fprintf(outfile, "%d", rowPtrs[compNum][0][blockNum][i]);
        }
...
int main() {
  ...
  // const int kSize = 256;
  // char* name = "V90423-201212.mp4";

  // char command[kSize];
  // snprintf(command, kSize, "ffmpeg -i %s image.jpg", name);
  // printf("%s", command);
  // system(command);

  //read_JPEG_file("image.jpg");
  read_DCT_coef("1.jpg", "dct_coefs.txt");
